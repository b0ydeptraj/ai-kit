...
This script processes multiple repositories (local or remote) using the repo-map CLI tool.
...
@dataclass
class RepomixConfig:
    """Configuration for repo-map execution."""
    style: str = "xml"
    output_dir: str = "repo-map-output"
...
class EnvLoader:
    ...
    def load_env_files() -> Dict[str, str]:
        ...
        search_paths = [
            ...
            script_dir.parent / ".env",                 # skill/.env (repo-map/.env)
        ]
...
class RepomixBatchProcessor:
    """Process multiple repositories with repo-map."""
    ...
    def check_repomix_installed(self) -> bool:
        """
        Check if repo-map is installed and accessible.

        Returns:
            True if repo-map is installed, False otherwise
        """
        try:
    ...
    def process_repository(
        ...
        """
        Process a single repository with repo-map.
    ...
    def _build_command(
        ...
        """
        Build repo-map command with configuration options.
        ...
        """
        cmd = ["npx" if is_remote else "repo-map"]

        if is_remote:
            cmd.extend(["repo-map", "--remote", repo_path])
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Batch process multiple repositories with repo-map"
    )
    ...
    parser.add_argument(
        "-o", "--output-dir",
        default="repo-map-output",
        help="Output directory (default: repo-map-output)"
    )
    ...
    # Check if repo-map is installed
    if not processor.check_repomix_installed():
        print("Error: repo-map is not installed or not in PATH", file=sys.stderr)
        print("Install with: npm install -g repo-map", file=sys.stderr)
        return 1
