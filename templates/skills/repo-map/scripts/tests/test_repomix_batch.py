...
class TestRepomixConfig:
    ...
    def test_default_values(self):
        ...
        assert config.output_dir == "repo-map-output"
...
class TestRepomixBatchProcessor:
    ...
    def test_check_repomix_installed_success(self, mock_run):
        """Test checking if repo-map is installed (success)."""
        ...
        assert args[0][0] == ["repo-map", "--version"]

    @patch("subprocess.run")
    def test_check_repomix_installed_failure(self, mock_run):
        """Test checking if repo-map is installed (failure)."""
    ...
    def test_check_repomix_installed_not_found(self, mock_run):
        """Test checking if repo-map is not found."""
...
class TestMain:
    ...
    def test_main_repomix_not_installed(self, mock_check):
        """Test main function when repo-map is not installed."""
        result = main()
        assert result == 1
