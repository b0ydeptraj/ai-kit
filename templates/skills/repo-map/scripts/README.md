...
Batch process multiple repositories (local or remote) using the repo-map CLI tool.
...
Requires Python 3.10+ and repo-map CLI:
...
options:
  ...
  -o, --output-dir DIR  Output directory (default: repo-map-output)
...
**Process remote repositories:**
```bash
python repomix_batch.py yamadashy/repo-map facebook/react --remote
```
