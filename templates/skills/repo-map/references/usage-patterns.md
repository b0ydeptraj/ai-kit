...
### Full Repository
```bash
repo-map --remove-comments --style markdown -o full-repo.md
```
...
### Focused Module
```bash
repo-map --include "src/auth/**,src/api/**" -o modules.xml
```
...
```bash
git checkout feature-branch && repo-map --include "src/**" -o feature.xml
git checkout main && repo-map --include "src/**" -o main.xml
```
**Use:** Feature branch review, change impact, before/after comparison, migration planning

### Cross-Repository
```bash
npx repo-map --remote org/repo1 -o repo1.xml
npx repo-map --remote org/repo2 -o repo2.xml
```
...
### Third-Party Library
```bash
npx repo-map --remote vendor/library --style xml -o audit.xml
```
...
### Pre-Deployment
```bash
repo-map --include "src/**,config/**" --style xml -o pre-deploy-audit.xml
```
**Checklist:** No sensitive data, no test credentials, env vars correct, security practices, no debug code

### Dependency Audit
```bash
repo-map --include "**/package.json,**/package-lock.json" -o deps.md --style markdown
repo-map --include "node_modules/suspicious-package/**" -o dep-audit.xml
```
**Use:** Suspicious dependency, security advisory, license compliance, vulnerability assessment

### Compliance
```bash
repo-map --include "src/**,LICENSE,README.md,docs/**" --style markdown -o compliance.md
```
...
### Doc Context
```bash
repo-map --include "src/**,docs/**,*.md" --style markdown -o doc-context.md
```
...
### API Documentation
```bash
repo-map --include "src/api/**,src/routes/**,src/controllers/**" --remove-comments -o api-context.xml
```
...
### Architecture
```bash
repo-map --include "src/**/*.ts,*.md" -i "**/*.test.ts" --style markdown -o architecture.md
```
**Focus:** Module structure, dependencies, design patterns, data flow

### Examples
```bash
repo-map --include "examples/**,demos/**,*.example.js" --style markdown -o examples.md
```
...
### Quick Assessment
```bash
npx repo-map --remote owner/library --style markdown -o library-eval.md
```
**Evaluate:** Code quality, architecture, dependencies, tests, docs, maintenance

### Feature Comparison
```bash
npx repo-map --remote owner/lib-a --style xml -o lib-a.xml
npx repo-map --remote owner/lib-b --style xml -o lib-b.xml
```
**Compare:** Features, API design, performance, bundle size, dependencies, maintenance

### Integration Feasibility
```bash
npx repo-map --remote vendor/library --include "src/**,*.md" -o library.xml
repo-map --include "src/integrations/**" -o our-integrations.xml
```
Analyze compatibility between target library and your integration points

### Migration Planning
```bash
repo-map --include "node_modules/old-lib/**" -o old-lib.xml
npx repo-map --remote owner/new-lib -o new-lib.xml
```
...
- name: Generate Snapshot
  run: |
    ...
    repo-map --style markdown -o release-snapshot.md
...
repo-map --include "$(cat staged-files.txt | tr '\n' ',')" -o .context/latest.xml
```

### IDE (VS Code)
```json
{"version": "2.0.0", "tasks": [{"label": "Package for AI", "type": "shell", "command": "repo-map --include 'src/**' --remove-comments --copy"}]}
```

### Claude Code
```bash
repo-map --style markdown --copy  # Then paste into Claude
```
...
### TypeScript
```bash
repo-map --include "**/*.ts,**/*.tsx" --remove-comments --no-line-numbers
```
**Exclude:** `**/*.test.ts`, `dist/`, `coverage/`

### React
```bash
repo-map --include "src/**/*.{js,jsx,ts,tsx},public/**" -i "build/,*.test.*"
```
**Include:** Components, hooks, utils, public assets

### Node.js Backend
```bash
repo-map --include "src/**/*.js,config/**" -i "node_modules/,logs/,tmp/"
```
**Focus:** Routes, controllers, models, middleware, configs

### Python
```bash
repo-map --include "**/*.py,requirements.txt,*.md" -i "**/__pycache__/,venv/"
```
**Exclude:** `__pycache__/`, `*.pyc`, `venv/`, `.pytest_cache/`

### Monorepo
```bash
repo-map --include "packages/*/src/**" -i "packages/*/node_modules/,packages/*/dist/"
```
...
**Fix:**
```bash
repo-map -i "node_modules/**,dist/**,coverage/**" --include "src/core/**" --remove-comments --no-line-numbers
```
...
repo-map --no-gitignore --no-default-patterns --verbose
```
...
```bash
repo-map --no-security-check  # Use carefully for false positives
```
...
**Optimize:**
```bash
repo-map --include "src/**/*.ts" -i "node_modules/**,dist/**,vendor/**"
```
...
**Fix:**
```bash
npx repo-map --remote https://github.com/owner/repo  # Full URL
npx repo-map --remote https://github.com/owner/repo/commit/abc123  # Specific commit
