...
> Historical note: this document is kept for migration/design traceability. Active runtime naming is `relay_kit.py`, `.relay-kit/`, and `.relay-kit-prompts/`.


| Capability | Current ai-kit v2 state | Added in this upgrade | Practical effect |
|---|---|---|---|
| Workflow entry / next-step help | No central router; users infer sequence manually | `workflow-router` + `.legacy-runtime/state/workflow-state.md` | Each request gets a named next step instead of vague guidance |
...
| Artifact handoffs | Little evidence of stable phase-to-phase contracts | `.legacy-runtime/contracts/` templates (`product-brief`, `PRD`, `architecture`, `epics`, `story`, `qa-report`, `tech-spec`) | Downstream roles inherit context instead of re-guessing it |
...
| Legacy support skill durability | `project-architecture`, `api-integration`, `data-persistence`, `testing-patterns` are prompt snippets in the old generator | Registry-native support skills + `.legacy-runtime/references/` templates | Architecture, API, persistence, and test knowledge become living shared references |
