...
- ignore policy for generated support, signal, and publication-evidence artifacts
...
The readiness command also includes a required local `release-lane` gate. `relay-kit publish plan` records the package-index evidence still needed before upload. `relay-kit publish evidence` records post-check/post-upload proof, but it does not upload artifacts itself.
