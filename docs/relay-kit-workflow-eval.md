...
- The bundled default suite covers 70 scenarios across routing, implementation, QA, release, policy, architecture, UX, dependency, API, data, media, browser evidence, multimodal evidence, bootstrap, debug/fix/review hubs, analyst, brainstorm-hub, scout-hub, team, execution-loop, testing-patterns, PM, scrum, runtime diagnostics, context governance, lane audit, adapter diagnostics, token economy, query lookup, service-boundary review, and the remaining utility workflows such as doc-pointers, repo-map, memory-search, handoff-context, research, problem-solving, root-cause-debugging, sequential-thinking, mermaid-diagrams, skill-evolution, skill-gauntlet, and test-first-development.
